module Fable.Flutter.AudioPlayers
...
type AudioPlayer() =
    ...
    member _.pause() : Future<unit> = nativeOnly
    /// Releases the resources associated with this media player.
    member _.release() : Future<unit> = nativeOnly
    [<NamedParams(fromIndex=1)>] member _.play(source: Source, [<OptionalArgument>] volume: float, [<OptionalArgument>] balance: float, (*[<OptionalArgument>] ctx: AudioContext,*) [<OptionalArgument>] position: System.TimeSpan, [<OptionalArgument>] mode: PlayerMode) : Future<unit> = nativeOnly
    /// Resumes the audio that has been paused or stopped.
    member _.resume() : Future<unit> = nativeOnly
    /// Moves the cursor to the desired position.
    member _.seek(position: System.TimeSpan) : Future<unit> = nativeOnly
    // member _.setAudioContext(ctx: AudioContext) : Future<unit> = nativeOnly
    /// Sets the stereo balance.
    member _.setBalance(balance: float) : Future<unit> = nativeOnly
    /// Sets the playback rate - call this after first calling play() or resume().
    member _.setPlaybackRate(playbackRate: float) : Future<unit> = nativeOnly
    /// Sets the release mode.
    member _.setReleaseMode(releaseMode: ReleaseMode) : Future<unit> = nativeOnly
    member _.setPlayerMode(mode: PlayerMode) : Future<unit> = nativeOnly
    /// Sets the audio source for this player.
    member _.setSource(source: Source) : Future<unit> = nativeOnly
    /// Sets the URL to an asset in your Flutter application. The global instance of AudioCache will be used by default.
    member _.setSourceAsset(path: string) : Future<unit> = nativeOnly
    member _.setSourceBytes(bytes: uint8 []) : Future<unit> = nativeOnly
    /// Sets the URL to a file in the users device.
    member _.setSourceDeviceFile(path: string) : Future<unit> = nativeOnly
    /// Sets the URL to a remote link.
    member _.setSourceUrl(url: string) : Future<unit> = nativeOnly
    /// Sets the volume (amplitude).
    member _.setVolume(volume: float) : Future<unit> = nativeOnly
    /// Stops the audio that is currently playing.
    member _.stop() : Future<unit> = nativeOnly